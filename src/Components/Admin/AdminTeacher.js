...
const AdminTeacher = () => {
  ...
  // these following three copied with modal for save new teacher .....
  ...
  const handleSave = () => {
    ...
    if (errors.name.length === 0 && errors.email.length === 0 && errors.address.length === 0 && errors.password.length === 0 && errors.mobile.length === 0) {
  ...
  /*========= get teacher by email ( To edit the teacher info )========== */
  ...
  const handleEdit = (email) => {
    handleShow();
    axios.get('http://localhost:8080/teacher/getbyemail/' + email).then((result) => {
  ...
  const handleUpdate = () => {
    const url = 'http://localhost:8080/teacher/update';
  ...
  const handleDelete = (email) => {
    Swal.fire({
      ...
      if (result.value) {
        axios.delete('http://localhost:8080/teacher/delete/' + email).then((result) => {
  ...
  return (
    <div>
          ...
          <div class="container-fluid table-container px-4">
            ...
            <div class="row my-5">
              ...
              <div className='mb-1 w-100 d-flex justify-content-between'>
                <h3 class="fs-4 mb-2">All Teachers</h3>
                <button className='mx-3 px-1 py-0 border-0 rounded-3' onClick={() => handleSaveShowFormn()}> Add Teacher <FontAwesomeIcon icon={faCirclePlus} /> </button>
                        ...
                        data && data.length > 0 ? data.map((item, index) => {
                          return (
                            <tr key={index}>
                              ...
                              <td colSpan={2}>
                                {/* edit button */}
                                <FontAwesomeIcon icon={faPenToSquare} className='me-2 fs-5 text-primary' onClick={() => handleEdit(item.email)} />
                                {/* delete button */}
                                <FontAwesomeIcon icon={faTrashCan} className='me-2 fs-5 text-danger' onClick={() => handleDelete(item.email)} />
                              </td>
                            </tr>
                          )
                        })
      ...
      {/* ================ Modal form to save the new Teacher details =============== */}


      <Modal show={showSave} onHide={handleSaveClose}>
        <Modal.Header closeButton>
          <Modal.Title>Save Teacher</Modal.Title>
        </Modal.Header>
        <Modal.Body>
      ...
      {/* ================ Modal form to update the Teacher details =============== */}
          ...
          <div className="row d-flex justify-content-center">
            <form className="col-8">
              <div className="mb-3">
                ...
                <input type="email" value={editEmail} readonly className="form-control" id="exampleInputPassword1" placeholder="Enter name" />
              </div>
              <div className="mb-3">
